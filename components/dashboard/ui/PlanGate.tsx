...
export function PlanGate({ children, requiredPlan = 'pro', feature, fallback, upgradeHref }: PlanGateProps) {
  ...
  const defaultFallback = (
    <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 md:p-12 text-center">
      ...
      <h3 className="text-xl font-bold text-white mb-2">{feature || 'This feature'} is locked</h3>
      <p className="text-slate-400 mb-6 max-w-md mx-auto">
        Upgrade to <span className="font-semibold text-white">{PLAN_NAMES[targetPlan]}</span> to unlock {feature?.toLowerCase() || 'this feature'} and more.
      </p>
      <a
