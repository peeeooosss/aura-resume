...
import {
  FileText, ArrowLeft, Star, Shield, Zap, Sparkles, ChevronDown,
...
export default function ResumeDetail({ params }: { params: { id: string } }) {
  ...
  if (!resume) {
    return (
      <div className="max-w-4xl mx-auto text-center py-20">
        ...
        <p className="text-slate-400 mb-6">The resume you're looking for doesn't exist.</p>
        <Link
          href="/resumes"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-medium rounded-xl transition-colors"
        >
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          {resume.atsScore !== null && (
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/50 rounded-xl border border-slate-700">
              ...
              <span className="text-lg font-bold text-white">{resume.atsScore}</span>
          ...
          {!resume.isPrimary && (
            <button
              ...
              className="p-2 rounded-xl bg-slate-800/50 hover:bg-slate-800 text-slate-400 hover:text-amber-400 transition-colors"
          ...
          <button
            ...
            className="p-2 rounded-xl bg-slate-800/50 hover:bg-slate-800 text-slate-400 hover:text-rose-400 transition-colors"
            title="Delete Resume"
          >
      ...
      {/* Tab Navigation */}
      <div className="flex gap-1 p-1 bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl overflow-x-auto">
        {tabs.map(tab => {
          const Icon = tab.icon;
          return (
            <button
          ...
          {resume.skills.length > 0 && (
            <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-slate-500 text-sm">({resume.skills.length})</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {resume.skills.map(skill => (
                  <span key={skill} className="px-3 py-1.5 text-sm bg-slate-800/50 border border-slate-700 text-slate-300 rounded-full hover:border-indigo-500/30 hover:text-indigo-300 transition-colors">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {resume.experience.length > 0 && (
            <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-slate-500 text-sm">({resume.experience.length} roles)</span>
              </div>
              <div className="space-y-4">
                {resume.experience.map(exp => (
                  <div key={exp.id} className="p-4 bg-slate-800/30 rounded-2xl">
            ...
            <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
              ...
              <div className="space-y-3">
                {resume.education.map(edu => (
                  <div key={edu.id} className="p-4 bg-slate-800/30 rounded-2xl flex items-center justify-between">
                    <div>
                    ...
                    <span className="text-slate-500 text-sm shrink-0">{edu.year}</span>
                  </div>
                ))}
              </div>
            </section>
      ...
      {activeTab === 'sections' && (
        <div className="space-y-3">
          <SectionCard
            ...
            itemCount={0}
          >
            <p className="text-slate-400 text-sm">{resume.rawText.split('\n')[0]}</p>
          </SectionCard>
          ...
          {resume.experience.length > 0 && (
            ...
            >
              <div className="space-y-3">
                {resume.experience.map(exp => (
                  <div key={exp.id} className="p-3 bg-slate-800/30 rounded-xl">
                    <p className="text-white font-medium">{exp.role} at {exp.company}</p>
                    <p className="text-slate-500 text-sm">{exp.startDate} – {exp.current ? 'Present' : exp.endDate}</p>
                    <p className="text-slate-400 text-sm mt-2">{exp.bullets.length} bullet points</p>
                  </div>
                ))}
              </div>
            </SectionCard>
          ...
          {resume.education.length > 0 && (
            ...
            >
              {resume.education.map(edu => (
                <div key={edu.id} className="p-3 bg-slate-800/30 rounded-xl">
          ...
          {resume.skills.length > 0 && (
            ...
            >
              <div className="flex flex-wrap gap-2">
                {resume.skills.map(s => (
                  <span key={s} className="px-2 py-1 text-xs bg-slate-800 text-slate-300 rounded-full">{s}</span>
                ))}
              </div>
            </SectionCard>
          )}
          ...
          {resume.certifications.length > 0 && (
            ...
            >
              {resume.certifications.map((cert, i) => (
                <p key={i} className="text-slate-300 text-sm py-1">{cert}</p>
              ))}
            </SectionCard>
          )}
          ...
          {resume.projects.length > 0 && (
            ...
            >
              {resume.projects.map(proj => (
                <div key={proj.id} className="p-3 bg-slate-800/30 rounded-xl">
          ...
          {resume.languages.length > 0 && (
            ...
            >
              {resume.languages.map((lang, i) => (
                <p key={i} className="text-slate-300 text-sm py-1">{lang}</p>
              ))}
            </SectionCard>
          )}
        </div>
        ...
        <div className="space-y-6">
          {resume.atsScore !== null && resume.atsBreakdown ? (
            <>
              <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-lg font-bold text-white">ATS Score Breakdown</h2>
                </div>
              ...
              {resume.redFlags.length > 0 && (
                <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-lg font-bold text-white">Red Flags</h2>
                  </div>
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-12 text-center">
              ...
              <h2 className="text-xl font-bold text-white mb-2">Not Analyzed Yet</h2>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-lg font-bold text-white">AI Actions</h2>
            </div>

            <div className="grid sm:grid-cols-2 gap-4">
              <button
                onClick={handleTailor}
                disabled={!isPro || tailoring}
                className={cn(
                  'relative p-6 rounded-2xl border text-left transition-all',
                  isPro
                    ? 'bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border-indigo-500/30 hover:border-indigo-500/50 hover:shadow-lg hover:shadow-indigo-500/10'
                    : 'bg-slate-800/30 border-slate-700 opacity-75 cursor-not-allowed'
                )}
              >
                {!isPro && (
                ...
                <div className="w-10 h-10 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center mb-3">
                  <Zap className="w-5 h-5 text-indigo-400" />
                </div>
                <h3 className="text-white font-semibold mb-1">Tailor to Job Description</h3>
                <p className="text-slate-400 text-sm">
                  {isPro ? 'Customize your resume to match a specific job posting' : 'Upgrade to Pro to unlock resume tailoring'}
                </p>
                {tailoring && (
                  <div className="flex items-center gap-2 mt-3">
                    <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />
                    <span className="text-indigo-400 text-sm">Tailoring...</span>
                  </div>
                )}
              </button>
              ...
              >
                ...
                <h3 className="text-white font-semibold mb-1">AI Rewrite</h3>
                <p className="text-slate-400 text-sm">
    ...
    <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {itemCount > 0 && (
            <span className="text-slate-500 text-sm">({itemCount})</span>
          )}
        </div>
        {expanded ? (
      ...
      {expanded && (
        <div className="px-5 pb-5 pt-0 space-y-3 border-t border-slate-800">
