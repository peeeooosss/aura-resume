    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="text-center py-16">
          ...
          <h1 className="text-3xl font-bold text-white mb-3">No Resumes Yet</h1>
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage, analyze, and optimize your resumes</p>
        </div>
        {!showUploader && (
          <button
            onClick={() => setShowUploader(true)}
      ...
      {showUploader && (
        <div
          ...
          className={`mb-8 bg-slate-900/80 backdrop-blur-xl border-2 border-dashed rounded-3xl p-8 text-center transition-colors ${
            dragging
        ...
        >
          <UploadIcon className={`w-12 h-12 mx-auto mb-4 ${selectedFile ? 'text-emerald-400' : 'text-slate-500'} transition-colors`} />
          {selectedFile ? (
            <>
...
function ResumeCard({ resume, isPrimary, onSetPrimary, onDelete }: any) {
  return (
    <div className={`relative bg-slate-900/80 backdrop-blur-xl border rounded-2xl p-5 transition-all ${isPrimary ? 'border-indigo-500/30 bg-indigo-500/5' : 'border-slate-800 hover:border-slate-700'}`}>
      {isPrimary && (
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          <button className="p-1.5 rounded-lg text-slate-500 hover:text-white hover:bg-slate-800 transition-colors" aria-label="View resume">
            <Eye className="w-4 h-4" />
          </button>
          <button className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-slate-800 transition-colors" aria-label="Delete resume">
            <Trash2 className="w-4 h-4" />
          </button>
        </div>
      </div>

      <h3 className="font-semibold text-white mb-1 truncate">{resume.fileName}</h3>
      <p className="text-slate-500 text-sm mb-4">Uploaded {formatDate(resume.createdAt)}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {resume.skills.slice(0, 4).map((skill) => (
          <span key={skill} className="px-2 py-0.5 text-xs bg-slate-800 text-slate-300 rounded-full">{skill}</span>
        ))}
      ...
      <div className="space-y-2 pt-4 border-t border-slate-800">
        <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-300 hover:text-white font-medium transition-colors">
          <Eye className="w-4 h-4" />
          View Details
        </button>
        ...
        {!isPrimary && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-300 hover:text-white font-medium transition-colors"
          >
        ...
        <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 transition-all">
