...
function parseMarkdownResume(text: string): ResumeSection[] {
  ...
  for (const line of lines) {
    ...
    if (h2Match) {
      const sectionName = h2Match[1].toUpperCase().replace(/\*/g, '').trim();
    ...
    if (h3Match) {
      if (['EXPERIENCE', 'EDUCATION', 'PROJECTS', 'CERTIFICATIONS'].includes(currentSection.title)) {
        flushSubHeader();
        currentSubHeader = h3Match[1].trim();
        currentSubLines = [];
        continue;
      }
    }
    ...
    if (sectionMap[upperTrimmed] && !trimmed.startsWith('##') && !trimmed.startsWith('###') && !trimmed.startsWith('#')) {
      pushCurrentSection();
      currentSection = {
...
export function generateOptimizedResumePDF(resumeText: string, metadata: { candidateName?: string; generatedAt: string }): jsPDF {
  ...
  for (const section of sections) {
    if (section.title === 'CONTACT') {
      ...
      doc.text(titleMap[section.title] || section.title, marginLeft, y);
      y += 1;
      doc.setDrawColor(30, 58, 138);
      doc.setLineWidth(0.3);
      doc.line(marginLeft, y, marginRight, y);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`ATS-Optimized Resume | Generated by Aura Resume | ${metadata.generatedAt}`, 105, 293, { align: 'center' });
  }
