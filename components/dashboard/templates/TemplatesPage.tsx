...
function TemplateCard({ template, onUse, onDelete, onCopy }: { template: Template; onUse: () => void; onDelete: () => void; onCopy: () => void }) {
  ...
  return (
    <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-colors">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex-1 min-w-0">
          <p className="text-slate-300 text-sm line-clamp-2">{template.jdText}</p>
          {template.jdUrl && (
        ...
        <div className="flex items-center gap-2">
          <button onClick={onCopy} className="p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded-xl transition-colors" title="Copy cover letter">
            <Copy className="w-4 h-4" />
          </button>
          <button onClick={onUse} className="p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded-xl transition-colors" title="Edit & use">
            <Edit className="w-4 h-4" />
          </button>
          <button onClick={onDelete} className="p-2 text-slate-500 hover:text-rose-400 hover:bg-slate-800 rounded-xl transition-colors" title="Delete">
...
function CoverLetterEditor({ template, onSave, onClose }: { template: Template | null; onSave: (letter: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-slate-800">
          <h2 className="text-xl font-bold text-white">Cover Letter Editor</h2>
        ...
        <div className="flex border-b border-slate-800">
          <button onClick={() => setActiveTab('letter')} className={cn('flex-1 px-4 py-3 text-sm font-medium border-b-2 transition-colors', activeTab === 'letter' ? 'border-indigo-500 text-white' : 'text-slate-500 hover:text-white')}>
            Cover Letter
          </button>
          <button onClick={() => setActiveTab('subject')} className={cn('flex-1 px-4 py-3 text-sm font-medium border-b-2 transition-colors', activeTab === 'subject' ? 'border-indigo-500 text-white' : 'text-slate-500 hover:text-white')}>
            Subject Lines
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {activeTab === 'letter' && (
            <div className="space-y-4">
              ...
              <textarea
                ...
                className="w-full min-h-[400px] bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none font-mono text-sm"
          ...
          {activeTab === 'subject' && (
            <div className="space-y-4">
              <label className="block text-sm font-medium text-slate-400">Email Subject Line</label>
              <input
                type="text"
                value={subjectLine}
              ...
              {template?.templates.subjectLines.slice(1).map((sl, i) => (
                <button key={i} onClick={() => setSubjectLine(sl)} className="w-full text-left px-4 py-2 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-300 hover:bg-slate-800 hover:text-white transition-colors">
                  {sl}
                </button>
              ))}
            </div>
        ...
        <div className="flex items-center gap-3 p-4 border-t border-slate-800">
          <button onClick={onClose} className="flex-1 px-6 py-3 bg-slate-800 rounded-xl text-white font-semibold hover:bg-slate-700 transition-colors">
            Cancel
          </button>
    ...
    return (
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white">Cover Letters</h1>
            <p className="text-slate-400 mt-1">Generate tailored cover letters from job descriptions</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-white">Cover Letters</h1>
          <p className="text-slate-400 mt-1">Generate AI-tailored cover letters from job descriptions</p>
        </div>
      </div>

      <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6">
        <h2 className="text-xl font-bold text-white mb-4">Generate New Cover Letter</h2>
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[120px] bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Job URL (optional)</label>
            <input
              type="url"
              value={newJDUrl}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Use Resume</label>
            <select
              value={resumeId}
      ...
      {templates.length > 0 ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">Your Cover Letters</h2>
            <span className="text-slate-500 text-sm">{templates.length} total</span>
          </div>
        ...
        <div className="text-center py-16 bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl">
          ...
          <p className="text-slate-400 mb-4">Generate your first AI-tailored cover letter above</p>
        </div>
      )}
      ...
      {showUpgrade && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-md w-full">
            ...
            <p className="text-slate-400 mb-6">Cover Letter Generator is a Pro feature. Upgrade to unlock it.</p>
